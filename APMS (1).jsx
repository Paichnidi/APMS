...
// ── auth config (change these to your real credentials) ───────────────────────
const ADMIN_USER = "alterra";
const ADMIN_PASS = "apms2025";
const DISCORD_INVITE = "https://discord.gg/your-invite-here"; // ← replace with real link
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080a0f",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",textAlign:"center",maxWidth:560}}>
        {/* logo */}
        <div style={{width:72,height:72,borderRadius:16,background:"linear-gradient(135deg,#e8c96d,#b8952d)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:"2rem",color:"#0a0c10",fontFamily:"monospace",margin:"0 auto 1.5rem"}}>A</div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080a0f",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem"}}>
      ...
      <div style={{position:"relative",width:"100%",maxWidth:380,animation: shake ? "shake 0.4s ease" : "none"}}>
        ...
        <button onClick={onBack} style={{background:"none",border:"none",color:"#555",cursor:"pointer",fontFamily:"monospace",fontSize:".75rem",letterSpacing:2,textTransform:"uppercase",marginBottom:"2rem",padding:0,display:"flex",alignItems:"center",gap:6}}>
          ← Back
        </button>
        ...
        <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:14,padding:"2.5rem"}}>
          ...
          <button onClick={attempt}
            style={{width:"100%",background:"#e8c96d",color:"#0a0c10",border:"none",borderRadius:7,padding:"11px",fontWeight:700,fontSize:".85rem",letterSpacing:1,textTransform:"uppercase",cursor:"pointer",fontFamily:"'Courier New',monospace"}}>
            Authenticate
          </button>
        </div>
      </div>
...
function generateID(memberNum, cellIdx, ftIdx) {
  const num = String(memberNum).padStart(3,"0");
  const cell = CELL_IDS[cellIdx] ?? "X";
  const ft   = FT_IDS[ftIdx]   ?? "XX";
  return `${num}${cell}${ft}`;
}

let _nextMember = 1;
function nextMemberNum() { return _nextMember++; }
...
const seed = (() => {
  ...
  const names = [
    "Jordan Reyes","Marcus Webb","Priya Nair","Sam Okafor","Elena Vasquez",
    "Tyler Huang","Nina Petrov","Chris Osei","Alana Moss","Devon Tran",
    "Kai Brennan","Leila Farouk","Owen Burke","Sasha Lin","Mia Castello",
    "Remy Dube","Finn O'Shea","Zara Ahmed","Cole Prentiss","Isla Mackay",
  ];
  names.forEach((name, i) => {
    const [ci, fi] = combos[i % combos.length];
    const num = nextMemberNum();
    members.push({
      id: generateID(num, ci, fi),
      memberNum: num,
      name,
      email: name.toLowerCase().replace(" ",".")+`@alterra.net`,
      ...
      joinDate: `202${2 + (i%3)}-0${(i%9)+1}-${String((i%28)+1).padStart(2,"0")}`,
      penalties: i === 3
        ? [{date:"2024-03-10",type:"Warning",note:"Missed 3 consecutive ops"}]
        : i === 7
        ? [{date:"2024-06-01",type:"Suspension",note:"Conduct violation"}]
        : [],
    });
  });
  return members;
...
const inp = {
  width:"100%",boxSizing:"border-box",background:"#1a1d28",border:"1px solid #2a2d3a",
  borderRadius:6,padding:"8px 12px",color:"#e2e8f0",fontSize:".9rem",fontFamily:"inherit",
  outline:"none",
};
...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{background:colors[status]+"22",color:colors[status],border:`1px solid ${colors[status]}44`,borderRadius:20,padding:"2px 10px",fontSize:".72rem",fontWeight:700,letterSpacing:1,textTransform:"uppercase",whiteSpace:"nowrap"}}>
      {status}
    </span>
  );
}
...
function RankBadge({ rank }) {
  ...
  return (
    <span style={{background:`hsl(${hue},60%,20%)`,color:`hsl(${hue},80%,75%)`,border:`1px solid hsl(${hue},60%,35%)`,borderRadius:4,padding:"2px 8px",fontSize:".72rem",letterSpacing:.5}}>
      {rank}
    </span>
  );
}
...
function MemberForm({ initial, onSave, onClose, usedIDs }) {
  const isEdit = !!initial;
  const [form, setForm] = useState(initial ?? {
    name:"", email:"", phone:"", age:"",
    cell:0, fireteam:0, rank:RANKS[0], status:"Active", joinDate: new Date().toISOString().slice(0,10),
  });
  ...
  return (
    <Modal title={isEdit ? "EDIT MEMBER" : "ADD MEMBER"} onClose={onClose}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem"}}>
        <Field label="Email">
          <input style={inp} value={form.email} onChange={e=>set("email",e.target.value)} placeholder="jane@alterra.net"/>
        </Field>
        <Field label="Phone">
          <input style={inp} value={form.phone} onChange={e=>set("phone",e.target.value)} placeholder="555-0000"/>
        </Field>
        <Field label="Age">
          <input style={inp} type="number" value={form.age} onChange={e=>set("age",e.target.value)} placeholder="25"/>
        </Field>
        <Field label="Join Date">
          <input style={{...inp,colorScheme:"dark"}} type="date" value={form.joinDate} onChange={e=>set("joinDate",e.target.value)}/>
        </Field>
        <Field label="Cell">
      ...
      <div style={{display:"flex",gap:"1rem",marginTop:"1.5rem",justifyContent:"flex-end"}}>
        ...
        <button onClick={handleSave} style={btn}>Save Member</button>
      </div>
    </Modal>
  );
}
...
function PenaltyForm({ member, onSave, onClose }) {
  ...
  return (
    <Modal title={`ADD PENALTY — ${member.id}`} onClose={onClose}>
      ...
      <Field label="Notes">
        <textarea style={{...inp,minHeight:80,resize:"vertical"}} value={note} onChange={e=>setNote(e.target.value)} placeholder="Describe the incident…"/>
      </Field>
      <div style={{display:"flex",gap:"1rem",marginTop:"1.5rem",justifyContent:"flex-end"}}>
        <button onClick={onClose} style={{...btn,background:"transparent",border:"1px solid #2a2d3a",color:"#888"}}>Cancel</button>
        <button onClick={()=>{if(!note.trim())return alert("Add a note");onSave({type,date,note});}} style={{...btn,background:"#dc2626"}}>Record Penalty</button>
      </div>
    </Modal>
  );
}
    ...
    <Modal title={`MEMBER — ${member.id}`} onClose={onClose}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem",marginBottom:"1.5rem"}}>
        {[
          ...
          ["Cell", CELLS[member.cell]],
          ["Fireteam", FIRETEAMS[member.fireteam]],
        ].map(([l,v])=>(
          <div key={l}>
        ...
        <div>
          <div style={{color:"#555",fontSize:".7rem",letterSpacing:2,textTransform:"uppercase",fontFamily:"monospace",marginBottom:4}}>Rank</div>
          <RankBadge rank={member.rank}/>
        </div>
        <div>
          <div style={{color:"#555",fontSize:".7rem",letterSpacing:2,textTransform:"uppercase",fontFamily:"monospace",marginBottom:4}}>Status</div>
...
const btn = {
  background:"#e8c96d",color:"#0a0c10",border:"none",borderRadius:6,
  padding:"8px 18px",fontWeight:700,fontSize:".82rem",letterSpacing:1,
...
function APMSDashboard({ onLogout }) {
  ...
  const [view, setView]           = useState("table"); // table | grid | org
  const [modal, setModal]         = useState(null);    // null | {type, data}
  const [activeTab, setActiveTab] = useState("members"); // members | org
  ...
  function addMember(form) {
    const num = nextMemberNum();
    const id  = generateID(num, Number(form.cell), Number(form.fireteam));
    setMembers(ms=>[...ms, {...form, id, memberNum: num, cell: Number(form.cell), fireteam: Number(form.fireteam), age: Number(form.age), penalties:[]}]);
    setModal(null);
  }

  function editMember(form) {
    setMembers(ms=>ms.map(m=>{
      if(m.id !== modal.data.id) return m;
      const id = generateID(m.memberNum, Number(form.cell), Number(form.fireteam));
      return {...m,...form,id,cell:Number(form.cell),fireteam:Number(form.fireteam),age:Number(form.age)};
    }));
    setModal(null);
  }

  function addPenalty(penalty) {
    setMembers(ms=>ms.map(m=>m.id===modal.data.id ? {...m,penalties:[...m.penalties,penalty]} : m));
    setModal(null);
  }

  function removeMember(id) {
    if(!confirm("Remove this member from APMS?")) return;
    setMembers(ms=>ms.map(m=>m.id===id ? {...m,status:"Removed"} : m));
  }

  const thStyle = (k) => ({
    padding:"10px 14px",textAlign:"left",color:"#555",fontSize:".7rem",letterSpacing:2,
    textTransform:"uppercase",fontFamily:"monospace",cursor:"pointer",whiteSpace:"nowrap",
    background: sort.key===k ? "#15182200" : "transparent",
    ...
    <div style={{minHeight:"100vh",background:"#080a0f",color:"#e2e8f0",fontFamily:"'Segoe UI',sans-serif"}}>
      ...
      <header style={{position:"sticky",top:0,zIndex:50,background:"#0a0c12ee",borderBottom:"1px solid #1a1d2a",backdropFilter:"blur(12px)",padding:"0 2rem"}}>
        <div style={{maxWidth:1400,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:60}}>
          <div style={{display:"flex",alignItems:"center",gap:"1rem"}}>
            <div style={{width:34,height:34,borderRadius:8,background:"linear-gradient(135deg,#e8c96d,#b8952d)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:".9rem",color:"#0a0c10",letterSpacing:0,fontFamily:"monospace"}}>A</div>
            <div>
      ...
      <main style={{maxWidth:1400,margin:"0 auto",padding:"2rem",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"1rem",marginBottom:"2rem"}}>
          {[
            {label:"Total Members", value:stats.total, color:"#e8c96d"},
            {label:"Active",        value:stats.active, color:"#22c55e"},
            {label:"Cells Active",  value:stats.cells, color:"#60a5fa"},
            {label:"With Penalties",value:stats.penalized, color:"#ef4444"},
          ].map(s=>(
        ...
        {activeTab === "members" && <>
          ...
          <div style={{display:"flex",gap:"1rem",marginBottom:"1.5rem",flexWrap:"wrap",alignItems:"center"}}>
            ...
            {[
              {label:"Cell", opts:["All",...CELLS], val:filterCell, set:setFilterCell},
              {label:"Fireteam", opts:["All",...FIRETEAMS], val:filterFT, set:setFilterFT},
              {label:"Status", opts:["All",...STATUSES], val:filterStatus, set:setFilterStatus},
            ].map(f=>(
            ...
            <div style={{marginLeft:"auto",display:"flex",gap:8}}>
              {["table","grid"].map(v=>(
                <button key={v} onClick={()=>setView(v)}
                  style={{background:view===v?"#1e2130":"transparent",border:"1px solid #2a2d3a",color:view===v?"#e2e8f0":"#555",borderRadius:6,padding:"6px 12px",cursor:"pointer",fontSize:".75rem",letterSpacing:1,textTransform:"uppercase"}}>
                  {v}
                </button>
              ))}
          ...
          <div style={{color:"#444",fontSize:".75rem",letterSpacing:1,marginBottom:"0.8rem"}}>
            ...
            <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:10,overflow:"hidden"}}>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  <thead>
                    <tr>
                      {[["id","ID"],["name","Name"],["email","Email"],["age","Age"],["cell","Cell"],["fireteam","Fireteam"],["rank","Rank"],["status","Status"],["joinDate","Joined"]].map(([k,l])=>(
                        <th key={k} style={thStyle(k)} onClick={()=>sortBy(k)}>
                  ...
                  <tbody>
                    {visible.length === 0 && (
                      <tr><td colSpan={10} style={{...tdStyle,textAlign:"center",color:"#444",padding:"3rem"}}>No members match the current filters.</td></tr>
                    )}
                    {visible.map(m=>(
                      <tr key={m.id} style={{transition:"background .15s"}}
                        onMouseEnter={e=>e.currentTarget.style.background="#13162099"}
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:"1rem"}}>
              ...
              {visible.map(m=>(
                <div key={m.id} style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:10,padding:"1.2rem",cursor:"pointer",transition:"border-color .2s"}}
                  onMouseEnter={e=>e.currentTarget.style.borderColor="#e8c96d44"}
                  ...
                  {Object.entries(fireteams).map(([ftIdx, ftMembers])=>(
                    <div key={ftIdx} style={{background:"#0a0c12",border:"1px solid #1e2130",borderRadius:8,padding:"0.8rem"}}>
                      <div style={{fontFamily:"monospace",color:"#60a5fa",fontSize:".75rem",letterSpacing:2,textTransform:"uppercase",marginBottom:8,display:"flex",justifyContent:"space-between"}}>
                        <span>{FIRETEAMS[ftIdx]}</span>
                        <span style={{color:"#555"}}>{FT_IDS[ftIdx]}</span>
                      </div>
                      {ftMembers.map(m=>(
                        <div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:"1px solid #13151f",cursor:"pointer"}}
                          onClick={()=>setModal({type:"detail",data:m})}>
                          <div>
      ...
      {/* ── MODALS ── */}
      {modal?.type === "add"    && <MemberForm onSave={addMember} onClose={()=>setModal(null)} usedIDs={members.map(m=>m.id)}/>}
      {modal?.type === "edit"   && <MemberForm initial={modal.data} onSave={editMember} onClose={()=>setModal(null)} usedIDs={members.map(m=>m.id)}/>}
      {modal?.type === "detail" && (
        <MemberDetail
          member={members.find(m=>m.id===modal.data.id)}
          onClose={()=>setModal(null)}
          onEdit={()=>setModal({type:"edit",data:members.find(m=>m.id===modal.data.id)})}
          onAddPenalty={()=>setModal({type:"penalty",data:modal.data})}
        />
      )}
      {modal?.type === "penalty" && (
        <PenaltyForm
